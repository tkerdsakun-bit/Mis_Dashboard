...
const EditRepairModal = () => {
  ...
  const ws2 = utils.aoa_to_sheet([
    ...
    ...categoryData,
    [''], // ✅ แก้จาก [] เป็น [''] เพื่อให้เป็น row ว่าง
  ...
  departments.forEach(dept => {
    ...
    const deptData = [
      ...
      ['จำนวนครั้งซ่อมทั้งหมด', deptAssets.reduce((sum: number, a) => { // ✅ ระบุ type ของ sum
  ...
  const repairSummaryData = assets
    .map(a => {
      ...
      const totalRepairCost = assetRepairs.reduce((sum: number, r) => sum + r.repair_cost, 0); // ✅ ระบุ type
      
      if (repairCount === 0) return null;
      
      const assetPrice = parseFloat(a.price.replace(/,/g, '')) || 1;
      ...
      return [
        ...
        totalRepairCost,
        assetPrice,
        parseFloat(((totalRepairCost / assetPrice) * 100).toFixed(2)) // ✅ แก้ type
      ];
    })
    .filter((item): item is (string | number)[] => item !== null) // ✅ Type guard
    .sort((a, b) => (b[4] as number) - (a[4] as number)); // ✅ Type assertion

  const totalRepairs = repairSummaryData.reduce((sum: number, item) => sum + (item[4] as number), 0); // ✅ แก้ type
  const totalRepairCost = repairSummaryData.reduce((sum: number, item) => sum + (item[5] as number), 0); // ✅ แก้ type
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
      <div className="bg-white rounded-3xl p-8 max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-slideUp">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">✏️ แก้ไขทรัพย์สิน</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex gap-4 pt-6">
            ...
            >
              {uploading ? '⏳ กำลังอัพโหลด...' : '✅ บันทึกการแก้ไข'}
            </button>
            <button 
              type="button" 
    ...
    <div className="bg-white rounded-3xl p-8 max-w-6xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-slideUp">
      ...
      <div className="space-y-4">
        {repairHistory.map((repair) => (
          <div 
            key={repair.id} 
            className="bg-gradient-to-r from-orange-50 to-red-50 border-2 border-orange-100 rounded-2xl p-6 hover:shadow-2xl transition-all"
          >
            {/* Header Section - ชื่อทรัพย์สิน + ปุ่ม */}
              ...
              <div className="flex items-center gap-3">
                ...
                <button 
                  onClick={() => {
                    ...
                    setShowRepairHistoryModal(false); // ปิด modal นี้ก่อน
                  }}
                  className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-xl font-semibold transition-all hover:scale-105 flex items-center gap-2"
                >
                  ✏️ แก้ไข
                </button>
              </div>
